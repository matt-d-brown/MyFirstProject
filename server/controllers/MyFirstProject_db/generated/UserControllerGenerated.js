...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findBytasks/:key", authorize([]), UserController.findBytasks);
  ...
  */
  ...
  /**
  * UserModel.findBytasks
  *   @description CRUD ACTION findBytasks
  *   @param Objectid key Id della risorsa tasks da cercare
  *
  */
  findBytasks: async (req, res) => {
    try {
      const result = await UserModel.findBytasks(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
